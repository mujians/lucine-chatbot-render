/**
 * 🌱 SEED AUTOMATED TEXTS
 * Popola il database con i testi automatici predefiniti
 */
...
const defaultTexts = [
  // OPERATOR
  {
    ...
    label: 'Saluto Operatore',
    category: 'operator',
    text: 'Ciao! Un attimo che controllo la conversazione e vedo come posso aiutarti.\nIntanto, fammi sapere se hai altre esigenze specifiche 😊',
    description: 'Messaggio automatico quando un operatore prende controllo della chat',
    isActive: true
  },
  {
    key: 'operator_connected',
    label: 'Operatore Connesso',
    category: 'operator',
    text: 'Perfetto! Ti sto connettendo con un operatore...\n\n⏱️ {operatorName} ti risponderà a breve. Attendi un momento.',
    description: 'Messaggio quando viene trovato un operatore disponibile',
    isActive: true
  },

  // QUEUE
  {
    key: 'queue_no_operators',
    label: 'Nessun Operatore Online',
    category: 'queue',
  ...
  {
    key: 'ticket_start',
    label: 'Inizio Creazione Ticket',
    category: 'ticket',
    text: '🎫 **Perfetto! Creiamo un ticket di supporto.**\n\nUn operatore ti ricontatterà appena disponibile per riprendere la conversazione.\n\n👤 **Per iniziare, come ti chiami?**\n\n💡 _Scrivi "annulla" in qualsiasi momento per tornare alla chat_',
    description: 'Messaggio di inizio del flusso di creazione ticket',
    isActive: true
  },
  {
    key: 'ticket_ask_name',
    label: 'Richiesta Nome',
    category: 'ticket',
    text: '👤 **Come ti chiami?**\n\nInserisci il tuo nome per proseguire.',
    description: 'Richiesta del nome dell\'utente',
    isActive: true
  },
  {
    key: 'ticket_ask_contact',
    label: 'Richiesta Contatto',
    category: 'ticket',
    ...
    description: 'Richiesta del metodo di contatto (variabili: {name})',
    isActive: true
  },
  {
    key: 'ticket_created',
  ...
  {
    key: 'closure_request',
    label: 'Richiesta Chiusura',
    category: 'closure',
    text: 'Posso aiutarti con qualcos\'altro?',
    description: 'Domanda dell\'operatore prima di chiudere la conversazione',
    isActive: true
  },
  {
    key: 'chat_continue',
    label: 'Continua Chat',
    category: 'closure',
    text: 'Certo! Dimmi pure, come posso aiutarti ancora.',
    description: 'Risposta quando l\'utente vuole continuare la chat',
    isActive: true
  },
  {
    key: 'chat_end_goodbye',
    label: 'Saluto Finale',
    category: 'closure',
  ...
  // GENERAL
  {
    key: 'ticket_already_exists',
    label: 'Ticket Già Esistente',
    category: 'general',
    text: '✅ **Hai già un ticket aperto: #{ticketNumber}**\n\nUn operatore ti contatterà a breve. Se vuoi aggiungere informazioni, scrivile pure qui.',
    description: 'Messaggio quando l\'utente ha già un ticket aperto (variabili: {ticketNumber})',
    isActive: true
  },
  {
    key: 'resume_welcome',
    label: 'Benvenuto Ripresa Chat',
    category: 'general',
    text: '🎫 **Benvenuto!**\n\nHai ripreso la conversazione del ticket #{ticketNumber}.\n\n{operatorStatus}',
    description: 'Messaggio di benvenuto quando si riprende una chat da link (variabili: {ticketNumber}, {operatorStatus})',
    isActive: true
  }
];

async function seedAutomatedTexts() {
  ...
  try {
    ...
    console.log(`\n✅ Successfully seeded ${defaultTexts.length} automated texts!`);
  } catch (error) {
    console.error('❌ Error seeding automated texts:', error);
    throw error;
  } finally {
